...
Router.get("/allFeedback", async (req, res) => {
  ...
  if (result) {
    try {
      const feedback = await find_all_Feedback();
      if (!feedback) {
        return res
          .status(202)
          .json({ success: false, message: "User not found" });
...
Router.post("/addFeedback", verifyToken, async (req, res) => {
  if (req.role.id === 1 || req.role.id === 3) {
    const { imageFeedback, description } = req.body;

    if (!imageFeedback || !description) {
      res.status(400).json({
        success: true,
        message: "Nhập thiếu thông tin",
      });
    } else {
      const imageFeedbackRe = await find_by_name_row_feedback(
        "imageFeedback",
        imageFeedback
      );
      if (imageFeedbackRe.length > 0) {
        res.status(400).json({
          success: false,
        ...
        try {
          const newFeedbackItem = new Feedback({
            imageFeedback,
            description,
          });
          const newFeedbackRe = await InsertFeedback(
            newFeedbackItem
          );
          if (newFeedbackRe) {
            res.status(200).json({
    ...
    if (!imageFeedback || !description) {
      ...
      if (imageFeedbackRe.length > 0) {
        ...
        try {
          ...
          const newFeedbackRe = await UpdateFeedback(
            newFeedbackItem,
            req.params.id
          );
          if (newFeedbackRe) {
